...
def add_agency(agency_id=None):
    ...
    if type(data) == list and len(data) > 0:
        ...
        for sub in data:
            ...
            agency = storage.get_or_create(Agency, **sub)
            if agency:
                agency.save()
                j += 1
            i += 1
        return jsonify({"message": f"[{j}/{i} agency added successfully"}), 201
...
def delete_bus_stop(bus_stop_id=None):
    ...
    """
    results = fetch_process(Agency, filter, bus_stop_id)
    if results:
...
def handle_agency(agency_id=None):
    """Handles agency endpoint.
    """
    agency_handlers = reach_endpoint([get_agency, add_agency,
                                 delete_bus_stop, update_agency])
